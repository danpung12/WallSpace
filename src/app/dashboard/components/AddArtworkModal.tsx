...
interface AddArtworkModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSave: (artwork: Omit<Artwork, 'imageUrl'> & { file: File | null }) => void;
  artworkToEdit: Artwork | null;
}

const AddArtworkModal: React.FC<AddArtworkModalProps> = ({ isOpen, onClose, onSave, artworkToEdit }) => {
  ...
  const isFormValid = 
    ...
    description.trim() !== '' &&
    (isEditMode || selectedFile); // In add mode, a file must be selected
  ...
  const handleSubmit = (e: FormEvent<HTMLFormElement>) => {
    ...
    onSave({
      id: artworkToEdit?.id || Date.now(), // Use existing ID or generate a new one
      title,
      dimensions,
      file: selectedFile,
    });
  ...
  const handleRemoveFile = () => {
    setSelectedFile(null);
    if (previewUrl && !isEditMode) { // In edit mode, don't clear the initial image on remove
        URL.revokeObjectURL(previewUrl);
        setPreviewUrl(null);
    } else if (isEditMode && previewUrl !== artworkToEdit.imageUrl) {
        URL.revokeObjectURL(previewUrl);
        setPreviewUrl(artworkToEdit.imageUrl); // Revert to original image
    }
  ...
  return (
    ...
    >
      <div
        className={`relative w-full max-w-2xl max-h-[90vh] p-6 sm:p-8 md:p-10 rounded-2xl shadow-xl border border-gray-200 bg-white text-gray-800 flex flex-col transform transition-all duration-300 ease-out ${isOpen ? 'opacity-100 translate-y-0 scale-100' : 'opacity-0 translate-y-4 scale-95'}`}
      ...
      >
                    ...
                    <div className="space-y-1 text-center">
                      ...
                      <div className="flex text-sm text-gray-600 justify-center gap-1">
                        <button
                          ...
                          className="relative cursor-pointer bg-white rounded-md font-medium text-[var(--primary-color)] hover:opacity-80 focus-within:outline-none focus-within:ring-2 focus-within:ring-offset-2 focus-within:ring-[var(--primary-color)] px-1"
                        >
                          <span>파일 업로드</span>
                        </button>
        ...
        <div className="flex-shrink-0 mt-8">
            <button 
                ...
                className="w-full rounded-xl h-14 text-lg bg-[#3E352F] text-white font-bold transition-colors disabled:bg-gray-300 disabled:cursor-not-allowed enabled:hover:bg-opacity-90"
            >
